...
KERNEL_PYTHON = Path(__file__).resolve().parents[1] / "python"
...
def _run_triton_import(
    *, package: str | None = None, extra_pythonpath: str | None = None
    ...
    env = os.environ.copy()
    pythonpath_entries = [str(KERNEL_PYTHON)]
    ...
    code = textwrap.dedent(
        """
        import json
        import tokenspeed_kernel._triton as tk_triton

        with tk_triton.redirect_triton_to_tokenspeed_triton():
            import triton as redirected_triton

        print(json.dumps({
            "triton_name": tk_triton.triton.__name__,
            "tl_name": tk_triton.tl.__name__,
            "redirected_name": redirected_triton.__name__,
        }, sort_keys=True))
        """
    )
...
def test_triton_import_defaults_to_tokenspeed_vendor_package():
    ...
    assert result == {
        "redirected_name": "tokenspeed_triton",
