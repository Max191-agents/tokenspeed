...
`ArgmaxInputs` represents argmax over the last dimension of logits. The common
rank-2 case is row-wise argmax, where the semantic result is the lowest column
index whose value is maximal in each row. Rank-1 logits are also supported for
single-vector fallback paths and produce a scalar selected index. By default
the generator plants a known dominant maximum in every logical row, so
correctness tests do not depend on accidental random maxima or ties. It can
also generate rows with two equal maxima to exercise the first-index tie rule,
or leave random logits untouched and derive expected indices from the
reference. When a test needs a specific winning token id, `planted_indices` can
fix the maximum location for all rows or provide one maximum location per row.
...
- rank-1 argmax logits represent exactly one logical row and return scalar
  expected/output indices
- NaN-focused argmax layouts are row-wise and require rank-2 logits
