...
This slice covers sum all-reduce, all-gather, and sum reduce-scatter. It does
not yet cover fused communication-plus-normalization kernels or expert
all-to-all dispatch.
...
all-reduce, all-gather, and reduce-scatter. The generated values map to those
APIs through a small rank-local adapter: each distributed worker generates the
same operation-level values, selects `rank_inputs[rank]`, invokes the backend
kernel, and compares against the reference result for that rank.
