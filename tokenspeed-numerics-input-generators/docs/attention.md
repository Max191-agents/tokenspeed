...
attention merge-state. TokenSpeed also exposes GDN QKV split and MLA K/V
pack+quantize helpers that map directly to `GDNQKVSplitInputValues` and
`MLAKVPackQuantizeFP8InputValues`, plus DeepSeek V4 sparse-prefill index helpers
that accept the tensors and scalar workspace parameters generated by
`DeepSeekV4SparsePrefillIndexInputs`. DeepSeek V4 paged-cache index helpers
similarly consume `DeepSeekV4PagedIndexInputs` through small adapters. The
